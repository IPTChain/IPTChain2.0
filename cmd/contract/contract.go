...
func makeDeployContractTransaction(signer *account.Account, codeStr string, language int) (string, error) {
	...
	tx, err := transaction.NewDeployTransaction(fc, signer.ProgramHash, "IPT", "1.0", "IPT user", "user@onchain.com", "test uint", types.LangType(byte(language)))
	if err != nil {
...
func NewCommand() *cli.Command {
	return &cli.Command{
		Name:        "contract",
		Usage:       "deploy or invoke your contracts ",
		Description: "you could deploy or invoke your contracts.",
		ArgsUsage:   "[args]",
		Flags: []cli.Flag{
			cli.BoolFlag{
				Name:  "deploy, d",
